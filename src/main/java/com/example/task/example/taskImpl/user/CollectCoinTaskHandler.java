...
import com.example.task.example.event.user.CollectCoinEvent;
...
public class CollectCoinTaskHandler extends AbstractTaskTriggerHandler {

    @Override
    public TaskEventEnum getTaskEventEnum() {
        return TaskEventEnum.COLLECT_COIN;
    }
    ...
    protected void calibrateTask(String roleId, UserTaskInfoEntity unfinishedTask, BasicAllTaskEntity basicAllTaskEntity, TaskEventChange applicationEvent) {
        CollectCoinEvent collectCoinEvent = (CollectCoinEvent) applicationEvent;
        boolean complete = unfinishedTask.getProgress() + collectCoinEvent.getNum() >= basicAllTaskEntity.getCond1();
        if (complete) {
            unfinishedTask.setStatus(1);
            unfinishedTask.setProgress(basicAllTaskEntity.getCond1());
        } else {
            unfinishedTask.setStatus(0);
            unfinishedTask.setProgress(unfinishedTask.getProgress() + collectCoinEvent.getNum());
        }
    }
}
